...
const INITIAL_COLUMNS = [
  ...
  { id: "role",       label: "Role",              responsive: "" },
  { id: "skills",     label: "Skills",            responsive: "" },
  { id: "experience", label: "Experience",        responsive: "" },
  { id: "linkedin", label: "LinkedIn",        responsive: "" },
  { id: "portfolio", label: "Portfolio",        responsive: "" },
  { id: "resume", label: "Resume",        responsive: "" },
  { id: "coverLetter", label: "Cover Letter",        responsive: "" },
...
export default function JobApplicationsTable({
  ...
  const renderCell = (col, item) => {
    switch (col.id) {
          ...
          <td key={col.id} className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
            <p className="flex gap-1 items-center cursor-pointer">
              ...
              {copied && item.id === copyId ? (
                ...
                <CopyIcon
                  onClick={ async () => { await fetchMessages(); handleCopyEmail(item.email, item.phone); setCopyId(item.id); }}
                  className="w-4 h-4 text-gray-500 cursor-pointer"
                />
              )}
            </p>
        ...
        return (
          <td key={col.id} className={`px-6 py-4 whitespace-nowrap text-sm text-gray-600 ${col.responsive}`}>
            <span
              ...
              onClick={() => { setShowTexts(true); setUsername(item.fullName); setNumber(formatPhoneNumber(item.phone)); }}
            >
              {item.phone}
            </span>
          </td>
      ...
      case "portfolio":
        return (
          <td key={col.id} className={`px-6 py-4 whitespace-nowrap text-sm text-gray-600 underline hover:text-blue-500 ${col.responsive}`}>
      ...
      case "linkedin":
        return (
          <td key={col.id} className={`px-6 py-4 whitespace-nowrap text-sm underline hover:text-blue-500 text-gray-600 ${col.responsive}`}>
